...
const upsertCartItem = async ({
  ...
  try {
    ...
    const noteQuery =
      noteQueryValue === "" ? { $in: ["", null, undefined] } : noteQueryValue;
...
const applyRemoveVoucher = async ({
  ...
  if (action === "apply_voucher") {
    ...
    if (!cart.vouchers.find((v) => v.toString() === voucher._id.toString())) {
...
const upsertGroupCartItem = async ({
  ...
  if (action === "update_item" || action === "remove_item") {
    ...
    const isSelf =
      itemToUpdate.participantId.toString() === participant._id.toString();
    if (!isOwner && !isSelf) {
  ...
  if (action === "update_item") {
    ...
    if (toppings !== undefined) {
      ...
      const existingToppings = await CartItemTopping.find({
        cartItemId: itemToUpdate._id,
      })
      ...
      finalToppingDetails = existingToppings
        .map((t) => t.toppingId)
        .filter(Boolean);
    }
  ...
  if (action === "add_item") {
    ...
    if (itemToUpsert) {
      ...
      const toppingsPricePerItem =
        existingToppingDetails.reduce((sum, t) => sum + (t?.price || 0), 0) ||
        0;
      ...
      const toppingsPricePerItem =
        toppingDetails.reduce((sum, t) => sum + (t.price || 0), 0) || 0;
      const finalLineTotal = (dishPrice + toppingsPricePerItem) * quantity;
...
const completeGroupCart = async (userId, cartId, payload) => {
  ...
  try {
    console.log("🧩 cart.storeId:", cart.storeId);
    console.log("🧩 Active store sockets JS:", storeSockets[cart.storeId]);
    ...
    console.log("📦 Emit newOrderNotification to store:", cart.storeId);
  } catch (e) {

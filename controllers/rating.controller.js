const {
  ...
  const getRatingsByStore = async (req, res) => {
    try {
      const data = await getRatingsByStoreService(req.user?._id, req.query);
      return ApiResponse.success(res, data, "Store ratings fetched successfully");
    } catch (err) {
      return ApiResponse.error(res, err);
    }
  };
  ...
  const replyToRating = async (req, res) => {
    try {
      const data = await replyToRatingService(req.user?._id, req.params.id, req.body);
